...
def _load_keybindings() -> List[Tuple[str, str, str]]:
    ...
    try:
        from rsvp_tui.keybindings import (  # type: ignore
            DEFAULT_BINDINGS, BINDING_DESCRIPTIONS,
        )
    except Exception:
        DEFAULT_BINDINGS, BINDING_DESCRIPTIONS = {}, {}
...
def render() -> str:
    ...
    parts.append(_p("Project management is exposed as a set of \\fBuv run\\fR "
    ...
    commands: List[Tuple[str, str]] = [
        ("read <file>",      "Read a book by file path (imports on first run). "
                             ...
                             "\\fB\\-\\-focus\\fR."),
        ("import <file>",    "Import a book into the library. Prints ID, word "
                             "count, and chapter count."),
        ("library",
    ...
    parts.append(_tag("\\fB\\-\\-version\\fR",
                      "Print the version of rsvp and exit."))
    parts.append(_tag("\\fB\\-\\-help\\fR",
                      "Print the grouped help (Reading, Library, Configuration, "
                      "Diagnostics, App)."))
    ...
    tasks: List[Tuple[str, str]] = [
        ...
        ("clean",      "Remove build/, dist/, eggs, bytecode, caches. "
    ...
    if palette:
        ...
        for cid, title in palette:
            parts.append(f".TP\n\\fB{cid}\\fR\n{title}\n")
        parts.append(_p(
            "Type to filter; \\fBEnter\\fR picks the top match; "
            "\\fBEsc\\fR dismisses."
        ))
    ...
    parts.append(_p(
        "Default location is \\fI$RSVP_HOME\\fR or \\fI~/.rsvp/\\fR:"
    ))
    ...
    parts.append(_tag("\\fBRSVP_HOME\\fR",
                      "Override the data directory (default \\fI~/.rsvp\\fR)."))
    parts.append(_tag("\\fBRSVP_NEW_UI\\fR",
                      "Force\\-enable the new SettingsScreen (set to \\fB1\\fR)."))
    parts.append(_tag("\\fBNO_COLOR\\fR",
                      "Disable ANSI colour in the helper scripts."))
    parts.append(_tag("\\fBPYTHONIOENCODING\\fR",
                      "Set to \\fButf\\-8\\fR when reading non\\-ASCII books."))
