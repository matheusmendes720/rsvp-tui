...
def test_safe_callback_returns_normal_value():
...
def test_safe_callback_swallows_exception_returns_default():
...
def test_safe_callback_surfaces_toast_when_possible():
...
def test_safe_callback_survives_when_notify_fails():
...
def test_safe_callback_works_without_app_attr():
...
def test_safe_callback_log_traceback_off(caplog):
...
def test_atomic_write_text_no_partial_file_on_failure(tmp_path, monkeypatch):
