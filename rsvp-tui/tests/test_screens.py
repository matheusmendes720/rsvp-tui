...
def test_new_ui_disabled_by_default(monkeypatch):
...
def test_new_ui_enabled_values(monkeypatch, value):
...
def test_new_ui_disabled_values(monkeypatch, value):
...
def test_figure_changed_carries_ids():
...
def test_figure_state_advanced_default_book_id():
...
def test_figure_completed_default_book_id():
    m = FigureCompleted()
    assert m.book_id is None


def test_config_changed_default_keys_empty():
    m = ConfigChanged()
    assert m.keys == ()


def test_book_opened_default_id_empty():
    m = BookOpened()
    assert m.book_id == ""
...
def test_fuzzy_score_empty_query_is_match():
    """Empty query matches everything with a low score."""
    assert _fuzzy_score("", "Next Figure") > 0


def test_fuzzy_score_exact_substring_high():
...
def test_fuzzy_score_no_match_returns_zero():
...
def test_default_commands_all_have_titles():
    """Every command has a non-empty title."""
    for c in DEFAULT_COMMANDS:
        assert c.title.strip()


def test_default_commands_contain_core_actions():
...
def test_palette_screen_accepts_custom_commands():
...
def test_palette_screen_defaults_to_default_commands():
...
def test_default_registry_id_order_is_stable():
...
def test_each_registry_figure_has_metadata():
...
def test_figure_changed_message_dispatch_contract():
...
def test_figure_state_advanced_message_dispatch_contract():
...
def test_figure_completed_message_dispatch_contract():
...
def test_all_screen_modules_importable():
